...
function getRemainingTime(ISOString: string | number | Date) {
...
export default function Tweet({ id, metadata, className }: any) {
  ...
  const formattedText = text
    .replace(regexToMatchURL, (match: string) => {
  ...
  const repliedTo =
    referenced_tweets && referenced_tweets.find((t: { type: string }) => t.type === "replied_to");
                ...
                {m.type == "video" || m.type == "animated_gif" ? (
                  video ? (
                    <video
                      ...
                      playsInline
                      src={
                        video.variants.length >= 3 ? video.variants[2].url : video.variants[0].url
                      }
                      itemType="video/mp4"
                    />
                  ) : (
        ...
        <a className="!no-underline" href={url_meta.unwound_url} target="_blank" rel="noreferrer">
          <div className="rounded-2xl overflow-hidden border border-gray-200 drop-shadow-sm mb-5">
            <BlurImage
              ...
              alt={url_meta.title}
              width={2048}
              height={url_meta.images[0].height * (2048 / url_meta.images[0].width)}
        ...
        <div className="mt-5">
          {polls.map(
            (poll: {
              ...
              end_datetime: string | number | Date;
            }) => {
                    ...
                    <>
                      ...
                      >
                        ...
                        <p className="!my-0 z-10">{`${(
                          (option.votes / (poll.total_votes as any)) *
                          100
                        )
                          .toFixed(1)
                          .replace(".0", "")}%`}</p>
                        <div
                          ...
                          style={{
                            width: `${Math.round(
                              (option.votes / (poll.total_votes as any)) * 100
                            )}%`,
                          }}
                        />
                      </div>
